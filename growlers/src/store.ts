...
const filter = () => {
  ...
  return store.taps.filter(({beverageName, abv}) => beverageName.match(searchRE) && abv < store.alcoholLimit).slice(0,15)
}
...
export const subscribe = (callback: (state: TapStore) => void):(() => void) => {
